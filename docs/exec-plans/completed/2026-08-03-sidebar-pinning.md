...
- 项目已置顶时，其内部置顶会话独立显示在项目上方，并从项目内部移除以避免重复。
...
| 置顶项目中的会话 | 会话独立显示在置顶项目上方，项目内不重复 | 通过；`01-project-session-pinned.png`，会话 y=204、项目 y=236、目标 href 仅出现 1 次 |
...
- 定向测试：2 个测试文件、13 条测试通过。
- 完整测试：`npm run test` 退出码 0。
- 真实浏览器：项目和会话置顶集合各 1 项，会话仅出现 1 次并位于项目上方。
