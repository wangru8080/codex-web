...
export function streamClaude(options: ClaudeStreamOptions): ReadableStream<string> {
  ...
  if (options.sessionRuntimePin === 'claude_code' && runtime.id !== 'claude-code-sdk') {
    throw new Error(
      ...
      + 'or switch this session to CodexWeb / Codex Runtime.',
    );
  }
    ...
    async start(controllerRaw) {
      ...
      try {
        ...
        const queryOptions: Options = {
          ...
          // Auto-allow all CodexWeb built-in MCPs. These are host-defined
          // in-process servers (createSdkMcpServer in claude-client.ts below)
          // that ship with CodexWeb — they're not third-party plugins and
        ...
        // MCP servers: pass explicitly provided config (e.g. from CodexWeb UI).
        ...
        if (!isHeartbeatMode && resolved.provider) {
          ...
          if (projectMcps) {
            ...
            // Existing entries (CodexWeb UI / placeholder-managed) take
        ...
        // See src/lib/harness/auto-invoke-accounting.ts for the contract.
        {
          ...
          let userExtensions: ReturnType<
            typeof import('@/lib/harness/user-codepilot-extensions').scanUserCodexWebExtensions
          > = [];
          let externalExtensions: ReturnType<
            typeof import('@/lib/harness/external-framework-harness').scanExternalFrameworkExtensions
          > = [];
          try {
            const { scanUserCodexWebExtensions } = await import(
              '@/lib/harness/user-codepilot-extensions'
            );
            userExtensions = scanUserCodexWebExtensions({
        ...
        // CodexWeb does NOT explicitly inject plugins — the SDK reads settingSources
        ...
        queryOptions.canUseTool = async (toolName, input, opts) => {
          // Auto-approve CodexWeb's own in-process MCP tools — they are internal
