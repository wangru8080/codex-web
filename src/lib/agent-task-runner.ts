...
import type { ScheduledTask, TaskRunStatus, PermissionProfile } from '@/types';
...
export async function ensureTaskBoundSession(task: ScheduledTask): Promise<string> {
  ...
  const inheritedPermissionProfile =
    (originSession?.permission_profile as PermissionProfile | undefined)
    || 'request_approval';
...
export async function runScheduledAgentTask(
  ...
  try {
    ...
    const { loadAllMcpServers, loadCodexWebMcpServers } = await import('./mcp-loader');
    ...
    // saw the keyword-injected CodexWeb built-ins, never the
