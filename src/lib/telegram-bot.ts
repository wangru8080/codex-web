/**
 * Telegram Bot notification module for CodexWeb.
 *
...
export async function notifyPermissionRequest(
  ...
  const msg = [
    ...
    ``,
    `⚠️ Please approve or deny in CodexWeb.`,
  ].filter(Boolean).join('\n');
...
/**
 * Build a status summary of current CodexWeb sessions.
 ...
 * Supports: /status — replies with current CodexWeb session status.
        ...
        if (command === '/start') {
          await callTelegramApi(botToken, 'sendMessage', {
            ...
            text: [
              `👋 <b>CodexWeb Bot</b>`,
              ``,
              `I'll send you notifications about your CodexWeb tasks.`,
              ``,
              `<b>Commands:</b>`,
