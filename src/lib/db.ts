...
export function getDb(): Database.Database {
  if (!db) {
    ...
    //
    // CODEXWEB_DISABLE_DB_MIGRATION_IN_TESTS (set by the unit-test
    ...
    if (!fs.existsSync(DB_PATH) && process.env.CODEXWEB_DISABLE_DB_MIGRATION_IN_TESTS !== '1') {
      const home = os.homedir();
      const oldPaths = [
        // Old Electron userData paths (app.getPath('userData'))
        path.join(home, 'Library', 'Application Support', 'CodexWeb', 'codepilot.db'),
        ...
        // Legacy name
        path.join(home, 'Library', 'Application Support', 'CodexWeb', 'claude-gui.db'),
...
function migrateDb(db: Database.Database): void {
  ...
  // *targeted* CodexWeb provider, so the thread can only safely
  ...
  // Codex Home 接管字段：本地会话绑定官方 session_id 后，运行时优先 resume 原 thread。
  if (!colNames.includes('codex_home_session_id')) {
    safeAddColumn(db, "ALTER TABLE chat_sessions ADD COLUMN codex_home_session_id TEXT NOT NULL DEFAULT ''");
  }
  if (!colNames.includes('codex_home_model_provider')) {
    safeAddColumn(db, "ALTER TABLE chat_sessions ADD COLUMN codex_home_model_provider TEXT NOT NULL DEFAULT ''");
  }
  if (!colNames.includes('codex_home_adopted_at')) {
    safeAddColumn(db, "ALTER TABLE chat_sessions ADD COLUMN codex_home_adopted_at TEXT NOT NULL DEFAULT ''");
  }
  ...
  if (!colNames.includes('permission_profile')) {
    safeAddColumn(db, "ALTER TABLE chat_sessions ADD COLUMN permission_profile TEXT NOT NULL DEFAULT 'request_approval'");
  }
  ...
  // That path is now supported (CodexWeb + Codex runtimes via the
...
export function createSession(
  ...
  ).run(id, title || 'New Chat', now, now, model || '', systemPrompt || '', wd, '', projectName, 'active', mode || 'code', wd, providerId || '', permissionProfile || 'request_approval', sourceValue);
...
export function updateCodexHomeBinding(
  id: string,
  binding: { sessionId: string; modelProvider: string; adoptedAt?: string },
): void {
  const db = getDb();
  const adoptedAt = binding.adoptedAt || new Date().toISOString();
  db.prepare(
    'UPDATE chat_sessions SET codex_home_session_id = ?, codex_home_model_provider = ?, codex_home_adopted_at = ? WHERE id = ?',
  ).run(binding.sessionId, binding.modelProvider, adoptedAt, id);
}
