/**
 * provider-presence.ts — Single-source-of-truth: "does CodexWeb itself have
 ...
 *     lives under the Claude Code CLI's ownership; CodexWeb treats CLI login
 *     state as "not our business". A user with only settings.json and no
 *     CodexWeb-level provider will be intercepted and asked to add one.
 *
 * Used by:
...
/**
 * True when a single DB provider has credentials CodexWeb can dispatch on.
 *
 * Checks (in order):
 *   1. api_key non-empty
 ...
 */
export function providerHasUsableCodexWebAuth(p: ApiProvider): boolean {
...
/**
 * True when CodexWeb has at least one provider the backend can dispatch to.
 *
 ...
 */
export function hasCodexWebProvider(): boolean {
  ...
  try {
    for (const p of getAllProviders()) {
      if (providerHasUsableCodexWebAuth(p)) return true;
    }
  } catch {
    return true;
  }
