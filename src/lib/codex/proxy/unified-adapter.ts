 ...
 * createGoogleGenerativeAI / etc.). CodexWeb's `createModel()` factory
...
import { createCodexWebBuiltinTools } from './builtin-bridge';
...
export function createUnifiedAdapter(family: string): ResponsesAdapter {
  return async (input, resolved): Promise<ProxyResult> => {
    ...
    const bridge = createCodexWebBuiltinTools({
...
export function buildProviderOptions(
  ...
  if (effort) {
    ...
    // Mapping mirrors how CodexWeb's native runtime maps effort →
