 ...
 *   2. look up the target CodexWeb provider in the DB
 ...
 * serves all three CodexWeb families (OpenAI-compatible, Anthropic-
...
export async function handleProxyRequest(
  ...
  if (virtual) {
    ...
    if (!dbProvider) {
      return makeErrorResult(
        'provider_not_found',
        `Target CodexWeb provider not found: ${input.targetProviderId}. (If this is a virtual provider that surfaces under runtime=codex_runtime, it must also be registered in VIRTUAL_PROVIDERS in adapter.ts.)`,
