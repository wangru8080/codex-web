 ...
 *
 * Phase 5 Phase 3 (2026-05-13). Wires the existing CodexWeb
 ...
 *      CodexWeb's existing format (`data: {"type":...,"data":...}\n\n`).
    ...
    return new ReadableStream<string>({
      async start(controller) {
        ...
        const closeStream = (extra?: { error?: string }) => {
          ...
          // watch table small while CodexWeb stays connected.
        ...
        try {
          ...
          if (!requestedProviderId || requestedProviderId === 'env') {
            throw new Error(
              'Codex Runtime requires an explicit provider. Pick a configured CodexWeb provider or Codex Account; the env (Claude Code default) provider is not supported under Codex Runtime.',
            );
          }
          ...
          // approval flow through CodexWeb's existing PermissionPrompt
          ...
          // ── CodexWeb built-in tool bridge subscription (Phase 5c) ──
          ...
          // Inject the CodexWeb Memory MCP as a streamable-HTTP server
          ...
          const threadParams = buildCodexThreadParams({
            ...
            // piece that lets the proxy mount CodexWeb built-in
            ...
            // when the user picked a CodexWeb provider as the
            // proxy target.
            sessionId,
          ...
          const existingMcpFingerprint =
            typeof existingRef?.metadata?.mcpConfigFingerprint === 'string'
              ...
              : '';
          const isCodexHomeRef = existingRef?.metadata?.origin === 'codex_home';
          const refMetadata = {
            providerId: requestedProviderId,
            mcpConfigFingerprint: mcpFingerprint,
            ...(isCodexHomeRef ? { origin: 'codex_home' as const } : {}),
          };
          let threadId: string;
          if (
            existingRef &&
            existingProviderBinding === requestedProviderId &&
            (isCodexHomeRef || existingMcpFingerprint === mcpFingerprint)
          ) {
            try {
              ...
              threadId = existingRef.token;
              if (isCodexHomeRef && existingMcpFingerprint !== mcpFingerprint) {
              ...
              if (isCodexHomeRef) {
                ...
                throw new Error(`Codex Home 接管会话恢复失败：${reason}`);
              }
            ...
            if (existingRef && isCodexHomeRef) {
              throw new Error(
                'Codex Home 接管会话必须通过官方 thread/resume 续聊，但当前 provider 绑定与请求不一致。',
              );
            }
