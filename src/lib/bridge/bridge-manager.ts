    ...
    case '/feishu': {
      ...
      switch (subcommand) {
        case 'start': {
          ...
          if (!config) {
            response = '❌ Feishu plugin not configured.\n\nPlease set App ID and App Secret in CodexWeb settings, or use /feishu auth.';
            break;
          }
        ...
        case 'auth': {
          ...
          const config = (plugin as FeishuChannelPlugin).getConfig?.();
          if (!config) {
            response = '❌ App credentials not configured.\n\nPlease configure App ID and App Secret in CodexWeb Settings → Bridge → Feishu.';
            break;
          }
          // Note: CodexWeb currently uses app-level bot tokens (no user OAuth)
          ...
          response = [
            ...
            '',
            'ℹ️ CodexWeb uses app-level bot tokens.',
        ...
        case 'doctor': {
          ...
          // Known limitations
          lines.push('');
          lines.push('Known Limitations (CodexWeb vs OpenClaw):');
