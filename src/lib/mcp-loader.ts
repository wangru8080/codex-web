 ...
 * We only manually pass servers that need CodexWeb-specific processing:
 * ${...} env placeholder resolution from the CodexWeb DB.
 *
...
/**
 * Load MCP servers that need CodexWeb-specific processing.
 *
 * Returns only servers with ${...} env placeholders that were resolved
 * against the CodexWeb DB. Returns undefined when no such servers exist
 * (the common case), letting the SDK load everything natively.
 *
 * Used by: route.ts, conversation-engine.ts — passed to streamClaude().
 */
export function loadCodexWebMcpServers(): Record<string, MCPServerConfig> | undefined {
  try {
 ...
 * NOT intended for passing to the SDK — use loadCodexWebMcpServers() instead.
 *
 ...
 * CodexWeb DB the same way loadAndMerge does. Disabled servers are
 * filtered out.
 *
...
export function loadProjectMcpServers(projectCwd: string | undefined): Record<string, MCPServerConfig> | undefined {
  ...
  try {
    ...
    // The CodexWeb MCP Manager UI persists per-server enable/disable state
