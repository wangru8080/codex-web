 ...
 *                                        CodexWeb + Codex Runtime (codepilot_only).
 *
...
export function getModelCompat(args: {
  ...
  switch (providerCompat) {
    case 'claude_code_ready':
      ...
      // model is reachable from CodexWeb Runtime too. Marking both lets a
    ...
    case 'claude_code_experimental':
      ...
      // (src/lib/claude-code-compat/) now lets CodexWeb Runtime speak the
    ...
    case 'openrouter_anthropic_skin':
      ...
      // codepilot_runtime_compatible — CodexWeb Runtime expects the
      // OpenAI-compat `/v1` skin URL form, and silently accepting an
      // Anthropic-shaped URL would route CodexWeb through the wrong
      ...
      reasons.codepilot_runtime =
        'OpenRouter Anthropic skin URL (/api) — switch to /v1 skin for CodexWeb Runtime';
      break;
    case 'codepilot_only':
    ...
    case 'codex_account':
      ...
      // reachable from ClaudeCode SDK or the CodexWeb Runtime
      // native loop — Codex owns the thread / turn / tool execution
      // shape. CodexWeb provider proxy (Phase 5 §provider proxy)
      // is the future channel for the reverse direction (CodexWeb
...
export function compatLabel(compat: ProviderRuntimeCompat, isZh: boolean): string {
  switch (compat) {
    ...
    case 'codepilot_only':           return isZh ? 'CodexWeb · Codex' : 'CodexWeb · Codex';
...
export function compatTooltip(compat: ProviderRuntimeCompat, isZh: boolean): string {
  switch (compat) {
    ...
    case 'codepilot_only':
      return isZh
        ? 'OpenAI 兼容协议，可在 CodexWeb Runtime 与 Codex Runtime 下使用；不支持 Claude Code Runtime（不会出现在其模型选择器中）'
        : 'OpenAI-compatible protocol — usable from CodexWeb Runtime and Codex Runtime; not supported by Claude Code Runtime (never shown in its picker)';
    case 'codex_account':
      return isZh
