 ...
 *      能直接调用 CodexWeb 的看板工具，请切到 CodexWeb 使用"
 ...
 *
 * Examples (zh):
 *   "当前引擎 (Codex) 暂不支持「看板操作」。如需使用，请切到 CodexWeb 或 Claude Code。"
 ...
 * Codex itself), and the list below ONLY describes whether CodexWeb
 * Harness capabilities can be injected.
 */
export const CODEX_ACCOUNT_HEADER_NOTE: BilingualText = {
  zh:
    'Codex 自带的插件 / Skills 由 Codex 自己管理，下方清单仅展示 CodexWeb 这一侧的内置能力是否能被注入或调用。当前默认服务商是 Codex Account，CodexWeb 工具桥在这条路径上不可用。',
  en:
    'Codex’s own plugins / Skills are managed by Codex itself. The list below only describes whether CodexWeb’s built-in Harness can be injected or called. Codex Account is your current default provider, so the CodexWeb tool bridge is not active on this path.',
};

// ─────────────────────────────────────────────────────────────────────
// Phase 5e round 8 (2026-05-18) — user-extensions section.
//
// The built-in capability matrix above describes CodexWeb's first-
// party capabilities (widget / memory / tasks / image / media /
// dashboard / cli_tools / assistant_buddy). User-defined extensions
// (MCP servers configured in CodexWeb Settings or project .mcp.json,
...
export const USER_EXTENSIONS_SUMMARY: Record<RuntimeId, UserExtensionsSummary> = {
  ...
  codex_runtime: {
    ...
    label: { zh: '用户自定义 MCP / Skills', en: 'User MCP / Skills' },
    description: {
      zh: '项目 CLAUDE.md 作为文本提示对模型仍可见；但用户自定义 MCP / Skills / 斜杠命令在 Codex 这条路径上无法调用。如需使用，请切到 Claude Code 或 CodexWeb。',
      en: 'Project CLAUDE.md is still visible to the model as a text prompt, but user-defined MCP servers / Skills / slash commands cannot be called on the Codex path. Switch to Claude Code or CodexWeb to use them.',
    },
  },
};
...
export const CAPABILITY_EXECUTABLE_RUNTIMES: Readonly<Record<string, readonly RuntimeId[]>> = {
  ...
  // assistant_buddy now executable on Claude Code + CodexWeb Native.
