 ...
 * `CODEXWEB_TOOL_MUTATION_LEVELS` below. Phase 5 follow-up work can
...
export type MutationLevel =
  ...
  | 'safe_read'
  /** Mutates state local to CodexWeb (DB row, in-memory cache, UI
   *  surface). No filesystem writes outside the user's media
   *  library; no shell execution; no external API calls beyond
   *  CodexWeb's own backend. Example: `codepilot_dashboard_pin`,
...
/**
 * Authoritative classification of every CodexWeb built-in tool.
 *
 ...
 */
export const CODEXWEB_TOOL_MUTATION_LEVELS: Readonly<Record<string, MutationLevel>> = {
...
export function shouldSkipPermission(toolName: string): boolean {
  ...
  const level = CODEXWEB_TOOL_MUTATION_LEVELS[toolName];
...
export function getMutationLevel(toolName: string): MutationLevel | undefined {
  ...
  return CODEXWEB_TOOL_MUTATION_LEVELS[toolName];
}
