 ...
 *      CodexWeb/Codex (which sets `cli_enabled=false`). When override
 ...
 *      Settings toggle that auto-pairs with CodexWeb/Codex defaults).
...
export function resolveRuntime(overrideId?: string, _providerId?: string): AgentRuntime {
  ...
  //    `agent_runtime` set to CodexWeb/Codex (which auto-sets
  ...
  if (overrideId && overrideId !== 'auto') {
    ...
    if (overrideId === 'claude-code-sdk') {
      throw new Error(
        ...
        + 'Install the Claude Code CLI, or switch this session to CodexWeb or Codex Runtime.',
      );
    }
  }
  ...
  //    /api/chat by hasCodexWebProvider(); if we reach this point the user
