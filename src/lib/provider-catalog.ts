...
export const VENDOR_PRESETS: VendorPreset[] = [
  ...
  // reachable from CodexWeb Runtime and Codex Runtime but NOT Claude Code
  ...
  // an official-OpenAI lineup for an arbitrary third-party gateway.
  {
    ...
    description: 'OpenAI-compatible chat API — provide URL, key and model (CodexWeb / Codex runtimes)',
    descriptionZh: 'OpenAI 兼容第三方 API — 填写地址、密钥和模型（用于 CodexWeb / Codex 运行时）',
  ...
  // NOT sdkProxyOnly — OpenAI-compatible reaches CodexWeb + Codex runtimes
  // via the AI SDK chat path, never the Claude Code subprocess.
  {
    key: 'cline-pass',
    name: 'ClinePass',
    description: 'ClinePass subscription — open coding models via the Cline API (CodexWeb / Codex runtimes)',
    descriptionZh: 'ClinePass 订阅 — 通过 Cline API 访问开源编程模型（用于 CodexWeb / Codex 运行时）',
    protocol: 'openai-compatible',
    authStyle: 'api_key',
  ...
  // `opencode-go/` prefix is OpenCode-config-only, not the API model field).
  {
    ...
    description: 'OpenCode Zen Go subscription — OpenAI-compatible models (CodexWeb / Codex runtimes)',
    descriptionZh: 'OpenCode Zen Go 订阅 — OpenAI 兼容模型（用于 CodexWeb / Codex 运行时）',
    protocol: 'openai-compatible',
    authStyle: 'api_key',
  ...
  // fine — it appends only `/messages` in that case — so the CodexWeb path was
...
export function findMatchingPresetForRecord(record: {
  ...
  // classifies it as `codepilot_only` (CodexWeb + Codex runtimes), not
