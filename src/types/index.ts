...
export type ChatSessionOrigin = 'codepilot' | 'codex_rollout';
...
export interface ChatSession {
   ...
   * codepilot_proxy` for one specific CodexWeb provider; resuming
  ...
  /** 官方 Codex Home 接管源 session_id；非空表示本地会话续接官方 thread。 */
  codex_home_session_id?: string;
  /** 接管时匹配的官方 session_meta.model_provider，仅用于过滤和展示。 */
  codex_home_model_provider?: string;
  /** 接管发生时间。 */
  codex_home_adopted_at?: string;
  project_name: string;
  /**
  ...
  /** 虚拟会话来源；Codex rollout 会话不会写入 CodexWeb SQLite。 */
  origin?: ChatSessionOrigin;
  /** 只读会话禁止重命名、删除、发送和 PATCH。 */
  read_only?: boolean;
  /** 官方 Codex session_id，仅用于 CODEX_HOME/sessions 只读桥接。 */
  codex_session_id?: string;
  /** 官方 Codex session_meta.model_provider。 */
...
export type PermissionProfile =
  ...
  | 'full_access'
  | 'config'
  /** 历史兼容值；新版 UI 读取后按 request_approval 展示。 */
  | 'default';
...
export interface PopoverItem {
   ...
   * funnels rendering through CodexWebIcon → HugeIcons.
   */
 ...
 *                               chat). Flows through CodexWeb Runtime AND
 ...
 *
 * Each Runtime adapter (ClaudeCode / CodexWeb / Codex) produces a
...
export interface RuntimeContextAccountingSnapshot {
   ...
   *     'codepilot_proxy'  — user-supplied provider via CodexWeb bridge (Phase 5e)
   *     'native_app_server' — app-server self-managed
   * - CodexWeb / ClaudeCode: typically omitted (single backend)
   */
