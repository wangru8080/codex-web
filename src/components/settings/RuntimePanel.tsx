 ...
 * to live under the "Claude CLI" sidebar entry plus a parallel CodexWeb
 ...
 *   3. CodexWeb Runtime card — capabilities / permissions / context
...
export function RuntimePanel(props: RuntimePanelProps = {}) {
  ...
  // Claude Code + CodexWeb Runtime as a peer engine. Polling
  ...
  const handleRuntimeChange = async (value: AgentRuntime) => {
    ...
    // needs the CLI subprocess. CodexWeb Runtime AND Codex Runtime
  ...
  const claudeCodeStatus: RuntimeStatusInfo = useMemo(() => {
    if (!connected) {
      return {
        ...
        impact: isZh
          ? "无法用 Claude Code 内核跑会话；选用后会自动回退到 CodexWeb"
          : "Sessions cannot run on Claude Code; selecting it falls back to CodexWeb",
        recovery: isZh
  ...
  const codexRuntimeStatus: RuntimeStatusInfo = useMemo(() => {
    ...
    if (codexAvailability.kind === "not_installed") {
      return {
        ...
        impact: isZh
          ? "Codex Runtime 整体无法启用：Codex 账户模型（gpt-5.5 等）和 CodexWeb 服务商经 proxy 接入两条路径都会发送失败"
          : "Codex Runtime is fully blocked: both Codex Account models (gpt-5.5 etc.) and CodexWeb providers via the proxy will fail at send time",
        recovery: isZh
    ...
    if (codexAvailability.kind === "spawn_failed") {
      return {
        ...
        impact: isZh
          ? "Codex Runtime 整体不可用（Codex 账户模型 + CodexWeb 服务商经 proxy 接入都受影响）；查看终端日志获取详细错误"
          : "Codex Runtime is fully unavailable (both Codex Account models and CodexWeb providers via the proxy are blocked); check terminal logs for details",
        recovery: isZh ? "点右上角刷新重试，或重启 CodexWeb" : "Click refresh in the top right, or restart CodexWeb",
      };
    }
    ...
    if (codexAvailability.kind === "ready") {
      return isSelected
        ? {
            ...
            impact: isZh
              ? "新会话默认走 Codex：Codex 账户模型 + 已配置 CodexWeb 服务商通过 provider proxy 接入（Claude Code 默认/env 模式除外）"
              : "New chats run on Codex: Codex Account models AND configured CodexWeb providers via the provider proxy (env Claude Code default is excluded)",
          }
        : {
            state: "available",
            reason: isZh ? "Codex 应用服务已就绪但未被设为默认" : "Codex app-server is ready but not the default engine",
            impact: isZh
              ? "想把 Codex 设为默认（同时启用 Codex 账户 + CodexWeb 服务商 via proxy），把上方「默认引擎」切到 Codex"
              : 'Switch the "Default engine" selector above to make Codex the default for both Codex Account models and CodexWeb providers via the proxy',
          };
    }
  ...
  /**
   * CodexWeb Runtime is bundled and always available; the only thing
  ...
  const codepilotStatus: RuntimeStatusInfo = useMemo(() => {
    if (effectiveRuntime === "native") {
      return {
        state: "selected",
        reason: isZh
          ? "CodexWeb 是默认内核（无需 CLI，直连 provider API）"
          : "CodexWeb is the default engine (no CLI required, direct provider API)",
        impact: isZh
          ? "新会话默认用 CodexWeb；工具、权限和上下文由 CodexWeb 自己管理"
          : "New chats run on CodexWeb; tools, permissions, and context managed by CodexWeb itself",
      };
    }
    return {
      state: "available",
      reason: isZh
        ? "CodexWeb 内核随应用自带，始终可用"
        : "CodexWeb ships with the app and is always available",
      impact: isZh
        ? "想切到 CodexWeb 内核，把上方「默认引擎」切到 CodexWeb 即可"
        : 'Switch the "Default engine" selector above to use CodexWeb',
    };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
        ...
        {driftWarning && (
          ...
          <div className="mb-3 rounded-md border border-status-warning-muted bg-status-warning-muted/30 px-3 py-2 text-[11px] text-status-warning-foreground flex items-start gap-1.5">
            ...
            <span>
              {!cliEnabled
                ? (isZh
                    ? "保存的偏好是 Claude Code，但 CLI 在「设置」里被显式关闭过，运行时实际走 CodexWeb。点上面任一卡片可一次写齐两边设置。"
                    : "Stored preference is Claude Code but CLI was explicitly disabled in a previous setting, so runtime actually routes to CodexWeb. Click either card above to rewrite both fields together.")
                : (isZh
                    ? "保存的偏好是 Claude Code，但当前没有检测到 Claude Code CLI（可能未安装或登录失效），运行时实际走 CodexWeb。下方 Claude Code 卡片提供安装入口；或者改选 CodexWeb 作为默认。"
                    : "Stored preference is Claude Code but the CLI isn't currently detected (not installed or OAuth expired), so runtime actually routes to CodexWeb. Use the Install button on the Claude Code card below — or pick CodexWeb as your default instead.")}
            </span>
          </div>
        )}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <EnginePickerCard
            ...
            statusText={connected
              ...
              : (isZh ? "未安装 — 选用后会自动降级到 CodexWeb" : "Not installed — selecting it falls back to CodexWeb")}
            isZh={isZh}
          ...
          <EnginePickerCard
            ...
            tagline={isZh ? "CodexWeb 自带内核" : "CodexWeb built-in"}
            pitch={isZh
              ? "CodexWeb 直连 provider API 跑 Agent。适合多 provider、可观察、可恢复，由 CodexWeb 自管上下文和权限，不依赖外部 CLI。"
              : "CodexWeb calls provider APIs directly. Built for multi-provider, observable, recoverable runs — context and permissions stay inside CodexWeb, no external CLI required."}
            statusKind="ok"
          ...
          <EnginePickerCard
            ...
            pitch={isZh
              ? "通过 Codex 应用服务调用 ChatGPT 账户内置模型（gpt-5.5 等，额度走 ChatGPT 套餐），同时已配置的 CodexWeb 服务商也能经 provider proxy 在 Codex 下使用（Claude Code 默认 / env 模式除外）。"
              : "Routes through the Codex app-server for Codex Account models (gpt-5.5 etc., quota covered by your ChatGPT plan), and also serves configured CodexWeb providers via the provider proxy (env Claude Code default is excluded)."}
            ...
            trigger={capabilityCells && (
              <RuntimeCapabilityList
                ...
                // and the list below ONLY describes whether CodexWeb’s
              ...
              <div className="py-2.5 flex items-center justify-between gap-3">
                ...
                <span className="text-xs text-status-warning-foreground text-right">
                  {!cliEnabled
                    ? (isZh
                        ? "CLI 已禁用 → 走 CodexWeb"
                        : "CLI disabled → routes to CodexWeb")
                    : (isZh
                        ? "Claude Code 不可用 → 自动用 CodexWeb"
                        : "Claude Code unavailable → falls back to CodexWeb")}
                </span>
              </div>
            )}
          </div>
      ...
      {/* ── CodexWeb detail card ────────────────────────────────────── */}
      <RuntimeCard name="CodexWeb" state={codepilotStatus.state} isZh={isZh}>
        ...
        <div className="rounded-md bg-muted/40 px-3.5 divide-y divide-border/50">
          ...
          <div className="py-2.5 flex items-start justify-between gap-3">
            <div className="flex flex-col gap-0.5 max-w-[55%]">
              ...
              <span className="text-[11px] text-muted-foreground leading-snug">
                {isZh
                  ? "CodexWeb 管理项目工作区、会话历史、模型选择和本地状态；自动按 token 预算修剪 / 压缩"
                  : "CodexWeb owns project workspace, session history, model choice, and local state; automatic token-budget prune + compress."}
              </span>
            </div>
            <span className="text-[10px] text-muted-foreground/70">
              {isZh ? "本地存储" : "local"}
