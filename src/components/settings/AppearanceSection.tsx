...
const MODE_OPTIONS: ReadonlyArray<{ value: string; icon: CodexWebIconName; labelKey: "settings.modeLight" | "settings.modeDark" | "settings.modeSystem" }> = [
    ...
    <div className="flex items-center rounded-lg border border-border/50 p-1 gap-1" role="radiogroup">
      {MODE_OPTIONS.map((opt) => {
        ...
        return (
          ...
          >
            <CodexWebIcon name={opt.icon} size="sm" aria-hidden />
            {t(opt.labelKey)}
          </Button>
        );
      })}
