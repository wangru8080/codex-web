...
describe("文件片段加入对话接线", () => {
  ...
  it("预览使用源 Markdown 和 frontmatter 偏移计算行号", () => {
    expect(preview).toContain("locateExcerptLines");
    expect(preview).toContain("sourceText={body}");
    expect(preview).toContain("lineOffset={lineOffset}");
    expect(preview).toContain("startLine: lines?.startLine");
    expect(preview).toContain("endLine: lines?.endLine");
  });
